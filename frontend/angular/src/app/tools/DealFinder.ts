...
// RUN MAIN FUNCTIONS. 
export function DealFinder({ charId }) {
...
function getIdOfActiveCharacter() {
  let characterId = fakeLocalStorage.getItem('mostRecentActiveCharacter');
  if (!characterId) {
...
//   const urlToken = urlParams && urlParams.access_token; // Initialize to boolean. Might be an object later.
//   const mostRecentAccessToken = JSON.parse(fakeLocalStorage.getItem('mostRecentAccessToken'));
...
function getCurrentLocation(characterId) {
  return new Promise((resolve, reject) => {
    ...
    getCharacterLocation(characterId)
        ...
        .then((data: any) => getConstellationInfo(data.constellation_id))
        .then((data) => resolve());
  });
}
...
function calcScores() {
  ...
  for (let typeId in itemData) {
    p.push(reprocessedValue(typeId).then((reprocessedValue: number) => {
...
function suggestOrders() {
  ...
  return new Promise((resolve, reject) => {
    ...
    const suggestedOrders = currentRoster;
